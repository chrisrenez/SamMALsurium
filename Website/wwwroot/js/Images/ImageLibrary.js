 ...
 */
function setupImageSelection(elements) {
    ...
    imageCards.forEach(card => {
        ...
        const selectForm = card.querySelector('form[action*="SelectFromLibrary"]');
        if (!selectForm) {
            return;
        }
 ...
 */
function setupPrivacyToggles(elements) {
    const { privacyForms } = elements;
    privacyForms.forEach(form => {
        const select = form.querySelector('select[name="privacy"]');
        if (!select) {
            return;
        }
 ...
 */
function updatePrivacyBadge(form, privacy) {
    const card = form.closest('[data-image-id]');
    if (!card)
        return;
    const badge = card.querySelector('.position-absolute.top-0.end-0 .badge');
    if (!badge)
        return;
    if (privacy === '0') {
        // Community Only
